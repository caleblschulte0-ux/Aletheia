...
def propose(args: dict, **_ignored) -> dict:
    ...
    # No `diff` from a session: the model asking is her local model, and code
    # proposals come from the subscriptions (CLAUDE.md). The record carries the
    # diagnosis and tests; `self_diagnosis propose --draft` asks for the diff.
    record = self_diagnosis.propose_patch(str(args.get("failure") or ""),
...
TOOLS = (
    ...
    tools.declare(
        "repo.propose_patch",
        description=("Record a proposed fix for a diagnosed defect for Caleb (failure; tests; "
                     "summary). Applies nothing and changes no code."),
        input_schema={"properties": {"failure": {"type": "string"},
