...
def resolve_device(endpoint: dict) -> int:
    ...
    if len(matches) > 1:
        # Count DEVICES, not distinct names. On this machine three separate
        # devices are all called "Speakers (Realtek(R) Audio)", and only one
        # of them opens at 16kHz — so identical names are the ambiguous case
        # that matters most, not the one to wave through.
        shown = ", ".join(f"{d['device_index']}:{d['name']}" for d in matches[:4])
        raise AudioDeviceError(
            f"endpoint {endpoint['id']!r} ({endpoint['label']!r}) matches "
            f"{len(matches)} devices ({shown}) — pin device_index in the plan "
