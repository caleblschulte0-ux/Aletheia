...
from aletheia import audio_router, calls, policy
...
def prepare(call_id: str, audio_session_id: str, *, audio_backend: audio_router.AudioBackend,
    ...
    value = {
        ...
        "call_approval_id": envelope["approval_id"],
        "audio_session_id": audio_session_id,
        "audio_plan_sha256": audio["plan_sha256"],
        "transport": _transport_id(transport),
        "state": "PREPARED",
...
def _revalidate(session: dict, *, audio_backend: audio_router.AudioBackend,
                transport: CallTransport) -> tuple[dict, dict]:
    policy.ensure_not_halted()
...
def dial(session_id: str, *, audio_backend: audio_router.AudioBackend,
    ...
    envelope, _ = _revalidate(session, audio_backend=audio_backend, transport=transport)
    # Persist an uncertain state BEFORE the external side effect. A crash after
    # this point requires observation/human reconciliation, never an auto-redial.
    ...
    try:
        policy.ensure_not_halted()
        observed = _normalize_observation(transport.dial(envelope))
...
def observe(session_id: str, *, audio_backend: audio_router.AudioBackend,
    ...
    if not session.get("call_handle"):
        # DIALING with no handle is the crash-uncertain state. Never dial again.
