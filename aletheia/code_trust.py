...
This grant is intentionally narrower than general workstation trust.  It lets a
local worker prepare reviewed pull requests for the operator's PUBLIC GitHub
repositories.  It never grants direct writes/merges to a default branch, private
repo code export, workflow/security-policy edits, secret access, or arbitrary
shell authority.
"""
...
def enable(*, days: int = DEFAULT_DAYS, max_prs: int = DEFAULT_PRS,
    ...
    record = {
        ...
        "max_prs": max_prs,
        "scope": (
            "public owner repos; branch+review+PR only; no default-branch merge/write, "
            "private code export, secrets, workflows or protected safety/governance paths"
        ),
    }
