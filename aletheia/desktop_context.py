"""Read-only foreground Windows context for screen perception.

This adapter observes which application owns the foreground window.  It does
not focus a window, send input, or persist the title/clipboard text.  Clipboard
capture is disabled by default and must be requested explicitly by a caller.
"""
...
class DesktopContextObservation:
    ...
    def process_name(self) -> str:
        if not self.process_path:
            return ""
        path_type = PureWindowsPath if "\\" in self.process_path else PurePath
        return path_type(self.process_path).name
    ...
    def metadata(self) -> dict:
        return {
            ...
            "window_title_sha256": hashlib.sha256(
                self.active_window_title.encode("utf-8")).hexdigest(),
    ...
    def reasoning_context(self, *, include_clipboard: bool = False) -> dict:
        value = {**self.metadata(), "active_window_title": self.active_window_title}
        if include_clipboard and self.clipboard is not None:
            value["clipboard"] = {
                **self.clipboard.metadata(), "text": self.clipboard.text}
        return value


class DesktopContextBackend(Protocol):
    def foreground(self) -> dict:
        """Return title/process_id/process_path for the foreground window."""
...
def capture(backend: DesktopContextBackend, *, now: dt.datetime | None = None,
            include_clipboard: bool = False) -> DesktopContextObservation:
    """Capture local context without reading the clipboard unless opted in."""
...
class WindowsContextBackend:
    """Stdlib/ctypes read-only implementation; no clicks, keys, or focus."""
    ...
    def __init__(self) -> None:
        ...
        self.kernel32.OpenProcess.argtypes = [wintypes.DWORD, wintypes.BOOL,
                                               wintypes.DWORD]
    ...
    def foreground(self) -> dict:
        ...
        return {"title": title_buf.value, "process_id": int(pid.value),
        ...
        try:
            ...
            try:
                ...
                if len(value) > MAX_CLIPBOARD_CHARS:
                    raise ValueError(
                        f"clipboard text exceeds {MAX_CLIPBOARD_CHARS} characters")
                return value
            finally:
