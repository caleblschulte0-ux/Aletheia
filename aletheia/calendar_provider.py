...
state before the action is called verified, and that provider verification is
persisted as an ActionRecord.
"""
...
from aletheia import calendar, policy, verification
...
def normalize_event(value: dict) -> dict:
    ...
    external_id = str(value["external_id"]).strip(); title = str(value["title"]).strip()
    if not external_id or not title:
        raise ValueError("provider external_id/title are required")
    start = calendar.parse_time(value["start"]); end = calendar.parse_time(value["end"])
    if end <= start:
    ...
    out = {"external_id": external_id, "title": title, "start": start.isoformat(),
           "end": end.isoformat(), "status": status, "attendees": list(dict.fromkeys(attendees))}
    for key in ("location", "etag", "updated_at"):
        if value.get(key) is not None: out[key] = str(value[key])
    return out
...
def _upsert_remote(provider_id: str, remote: dict) -> tuple[dict, str]:
    ...
    existing = next((e for e in calendar.all_events() if _provider_event(e, provider_id, remote["external_id"])), None)
    if existing and existing.get("provider_dirty"):
        return existing, "CONFLICT_LOCAL_DIRTY"
    now = utcnow()
    if existing:
        value = dict(existing); action = "UPDATED"
    else:
        value = {"version": 1, "id": _local_id(provider_id, remote["external_id"]),
                 "created_at": now, "priority": 3, "movable": False}; action = "CREATED"
    value.update({"title": remote["title"], "start": remote["start"], "end": remote["end"],
                  "attendees": remote.get("attendees", []), "status": remote["status"],
                  "source": f"provider:{provider_id}", "provider_id": provider_id,
                  "external_id": remote["external_id"], "provider_etag": remote.get("etag", ""),
                  "provider_updated_at": remote.get("updated_at", ""), "provider_dirty": False,
                  "synced_at": now, "updated_at": now})
    if remote.get("location"): value["location"] = remote["location"]
    elif "location" in value: value.pop("location")
    calendar.save(value); return value, action


def sync_window(provider: CalendarProvider, start: str, end: str, *, authoritative: bool = False) -> dict:
    """Import one bounded provider window; absence is non-authoritative by default."""
    a, b = calendar.parse_time(start), calendar.parse_time(end)
    if b <= a: raise ValueError("sync window end must be after start")
    provider_id = str(getattr(provider, "provider_id", "")).strip()
    if not provider_id: raise ValueError("calendar provider must declare provider_id")
    ...
    for remote in remote_values:
        seen.add(remote["external_id"]); local, action = _upsert_remote(provider_id, remote)
    ...
    if authoritative:
        for local in calendar.all_events():
            if local.get("provider_id") != provider_id or local.get("provider_dirty"): continue
            when = calendar.parse_time(local["start"])
            if a <= when < b and local.get("external_id") not in seen and local["status"] != "CANCELLED":
                local = dict(local); local["status"] = "CANCELLED"; local["updated_at"] = utcnow(); local["synced_at"] = utcnow(); calendar.save(local)
                actions.append({"external_id": local["external_id"], "local_id": local["id"], "action": "CANCELLED_MISSING_REMOTE"})
...
def build_write_plan(action: str, provider_id: str, *, event: dict | None = None,
    ...
    action = action.upper()
    if action not in ACTIONS: raise ValueError(f"calendar action must be one of {sorted(ACTIONS)}")
    if not isinstance(provider_id, str) or not provider_id.strip(): raise ValueError("provider_id is required")
    plan = {"version": 1, "action": action, "provider_id": provider_id.strip()}
    if action in {"UPDATE", "CANCEL"}:
        if not isinstance(external_id, str) or not external_id.strip(): raise ValueError(f"{action} requires external_id")
        plan["external_id"] = external_id.strip()
    if action in {"CREATE", "UPDATE"}:
        if not isinstance(event, dict): raise ValueError(f"{action} requires event")
        candidate = {"external_id": external_id or "pending-create", "title": event.get("title"),
                     "start": event.get("start"), "end": event.get("end"),
                     "status": event.get("status", "CONFIRMED"), "attendees": event.get("attendees", [])}
        if event.get("location") is not None: candidate["location"] = event["location"]
        normalized = normalize_event(candidate); normalized.pop("external_id", None); plan["event"] = normalized
    plan["sha256"] = _hash({k: v for k, v in plan.items() if k != "sha256"}); return plan
...
def execute_write_plan(plan: dict, approval_id: str, provider: CalendarProvider) -> dict:
    ...
    action = plan["action"]
    if action == "CREATE": observed = provider.create_event(dict(plan["event"]))
    elif action == "UPDATE": observed = provider.update_event(plan["external_id"], dict(plan["event"]))
    elif action == "CANCEL": observed = provider.cancel_event(plan["external_id"])
    else: raise ValueError("unsupported calendar write action")
    normalized = normalize_event(observed)
    if action in {"CREATE", "UPDATE"}:
        requested = plan["event"]
        for key in ("title", "start", "end", "status", "attendees"):
            if normalized.get(key) != requested.get(key): raise RuntimeError(f"provider verification failed for {key}")
    elif normalized["status"] != "CANCELLED": raise RuntimeError("provider did not verify cancellation")
    local, sync_action = _upsert_remote(plan["provider_id"], normalized)
    summary = f"{action} {normalized['external_id']} provider state matched approved plan"
    action_record = verification.execution_record(
        "calendar.write", provider=plan["provider_id"], intent=f"{action.lower()} calendar event",
        plan=plan, succeeded=True, result_summary=summary,
        evidence=[{"id":"provider-state","kind":"truthy","observed":True,"source":plan["provider_id"]}],
        requested_by="operator", approval_id=approval_id, policy_decision="APPROVED",
        reversible=action != "CREATE", action_id=verification.new_action_id("calendar.write", seed={"approval":approval_id,"plan":expected}))
    return {"outcome": "VERIFIED", "provider": plan["provider_id"], "external_id": normalized["external_id"],
            "local_id": local["id"], "sync_action": sync_action, "plan_sha256": expected,
            "action_record": action_record["id"], "verification_status": action_record["status"]}
...
class InMemoryCalendarProvider:
    ...
    def __init__(self, provider_id: str = "fake.calendar", events: list[dict] | None = None):
        ...
        self._events = {normalize_event(e)["external_id"]: normalize_event(e) for e in (events or [])}; self._next = 1
    def list_events(self, start: str, end: str) -> list[dict]:
        a, b = calendar.parse_time(start), calendar.parse_time(end)
        return [dict(e) for e in self._events.values() if a <= calendar.parse_time(e["start"]) < b]
    def create_event(self, event: dict) -> dict:
        external_id = f"fake-{self._next}"; self._next += 1
        value = normalize_event({"external_id": external_id, **event}); self._events[external_id] = value; return dict(value)
    def update_event(self, external_id: str, event: dict) -> dict:
        if external_id not in self._events: raise KeyError(external_id)
        value = normalize_event({"external_id": external_id, **event}); self._events[external_id] = value; return dict(value)
    def cancel_event(self, external_id: str) -> dict:
        if external_id not in self._events: raise KeyError(external_id)
        value = dict(self._events[external_id]); value["status"] = "CANCELLED"; self._events[external_id] = value; return dict(value)
