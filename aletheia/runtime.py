...
consumption. The runtime never expands authority.
"""
...
def _schedule_verification(spec: dict, receipt: dict) -> tuple[str | None, str | None]:
    ...
    try:
        verification.begin("automation.execute", provider="aletheia.local",
                           intent=f"scheduled {spec['command']['kind']}", plan=plan,
                           requested_by="scheduler", action_id=action_id,
                           inputs_summary=f"schedule {spec['id']}")
...
def _finish_schedule_verification(action_id: str | None, *, succeeded: bool,
    ...
    try:
        value = verification.record_execution(
            ...
            evidence=([{"id":"occurrence-receipt","kind":"truthy","observed":bool(receipt.get("occurrence")),
                        "source":"scheduler"}] if succeeded else []),
            auto_verify=False,
        )
        return value["status"]
    except Exception as exc:
...
def run_due_schedules(fleet: dict, *, now: dt.datetime | None = None, request=None) -> list[dict]:
    ...
    for spec in scheduler.all_schedules():
        ...
        if problems:
            results.append({"schedule": spec["id"], "outcome": "invalid", "detail": "; ".join(problems)})
            continue
        if policy.halted() and spec["command"]["kind"] != "resume":
            results.append({"schedule": spec["id"], "outcome": "halted", "detail": "kill switch is on"})
            continue
        ...
        try:
            ...
            result["action_record"] = action_id
...
def poll_mail_events() -> list[dict]:
    ...
    try:
        ...
        notifications.publish(
            "Mail polling failed", f"{type(exc).__name__}: {exc}", priority="IMPORTANT",
            source="mail", dedupe_key=f"mail-poll-error:{type(exc).__name__}", related={"capability":"email.read"})
    ...
    for transition in pulse.get("transitions", []):
        ...
        try:
            emitted = events.emit(
                ...
                source="pulse", attributes={"repo": transition["repo"], "from": transition["from"],
            ...
            actions.append({"action":"already_emitted","event":event_id,"repo":transition["repo"]})
...
def process_new_events(*, now: dt.datetime | None = None,
    ...
    for event in fresh:
        ...
        triggers = []; triggers_root = watchers_dir / "triggers"
        ...
        for trigger in triggers:
            notifications.publish("Watched event", f"{trigger['summary']} ({event['kind']}, {event['subject']})",
        ...
        for rule in rules:
            ...
            notifications.publish("Proactive: " + rule["id"], f"{event['summary']} ({event['kind']}, {event['subject']})",
    ...
    if fresh:
        write_json_atomic(cursor_path, {"version": 1, "last_event_id": fresh[-1]["id"], "updated_at": now.strftime("%Y-%m-%dT%H:%M:%SZ")})
    return actions
...
def tick(fleet: dict, *, now: dt.datetime | None = None, registry: dict | None = None, request=None) -> dict:
    ...
    for result in schedules:
        if result["outcome"] in {"refused", "error", "invalid"}:
            notifications.publish(f"Schedule {result['schedule']} {result['outcome']}", result["detail"],
    ...
    mail_events = poll_mail_events()
    pulse_events = mirror_pulse_events()
    reply_transitions = evaluate_replies(now=now)
    event_actions = process_new_events(now=now)
    return {
        "schedules": schedules,
        "mail_events": mail_events,
        "pulse_events": pulse_events,
        "reply_transitions": reply_transitions,
        "events_processed": event_actions,
