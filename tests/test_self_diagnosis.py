...
class Proposing(unittest.TestCase):
    ...
    def test_a_proposal_is_a_record_and_nothing_else(self):
        ...
        self.assertEqual(saved["provenance"]["diff_by"], "supplied by the caller")
    ...
    def test_a_boundary_gets_no_diff_and_says_why(self):
        ...
        record = sd.propose_patch("apply-diag0003", diagnosis=d, write=False,
                                  drafter=lambda s, t: drafted.append(1) or ({"diff": "x"}, "subscription"))
        self.assertEqual(drafted, [])
        self.assertIn("boundary", record["why_no_diff"])

    def test_the_subscriptions_draft_the_diff_and_are_named(self):
        diff = "--- a/aletheia/formfill.py\n+++ b/aletheia/formfill.py\n@@ -1 +1 @@\n-x\n+y\n"
        with mock.patch.object(repo_tools, "_tracked_file", lambda n: n):
            record = sd.propose_patch("apply-diag0002", diagnosis=self.diagnosis, write=False,
                                      drafter=lambda s, t: ({"diff": diff, "summary": "guard the key"},
                                                            "subscription"),
                                      check=lambda d: (False, "patch does not apply"))
        self.assertEqual(record["provenance"]["diff_by"], "subscription")
        self.assertFalse(record["diff_check"]["ok"])

    def test_repositories_stay_with_the_subscriptions(self):
        # CLAUDE.md: code proposals never come from her local model, whether it
        # drafts the diff or hands one in.
        diff = "--- a/aletheia/formfill.py\n+++ b/aletheia/formfill.py\n@@ -1 +1 @@\n-x\n+y\n"
        with mock.patch.object(repo_tools, "_tracked_file", lambda n: n):
            drafted = sd.propose_patch("apply-diag0002", diagnosis=dict(self.diagnosis), write=False,
                                       drafter=lambda s, t: ({"diff": diff}, "ollama:qwen3:8b"),
                                       check=lambda d: (True, ""))
            handed = sd.propose_patch("apply-diag0002", diff=diff, drafted_by="ollama:qwen3-vl:4b",
                                      diagnosis=dict(self.diagnosis), write=False, check=lambda d: (True, ""))
        self.assertEqual(drafted["diff"], "")
        self.assertEqual(handed["diff"], "")
        with mock.patch.object(sd, "propose_patch", wraps=sd.propose_patch) as spy, \
             mock.patch.object(sd, "diagnose", return_value=dict(self.diagnosis)):
            from aletheia import memory_tools
            memory_tools.propose({"failure": "apply-diag0002", "diff": diff})
        self.assertNotIn("diff", spy.call_args.kwargs)
        import inspect
        self.assertNotIn("local_think", inspect.getsource(sd.propose_patch))
