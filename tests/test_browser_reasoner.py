...
class BrowserReasonerCase(unittest.TestCase):
    ...
    def test_disabled_by_default_before_browser_is_even_probed(self):
        with mock.patch.dict(browser_reasoner.os.environ, clear=False):
            browser_reasoner.os.environ.pop(browser_reasoner.OPT_IN_ENV, None)
            with mock.patch.object(browser_reasoner.browse, "available") as available:
    ...
    def test_explicit_opt_in_is_required_and_recognized(self):
        for value in ("1", "true", "YES", "on"):
            with self.subTest(value=value), mock.patch.dict(
                    ...
                    {browser_reasoner.OPT_IN_ENV: value}, clear=False):
                self.assertTrue(browser_reasoner.enabled())

    def test_false_opt_in_values_stay_disabled(self):
        for value in ("", "0", "false", "off", "foreground"):
            with self.subTest(value=value), mock.patch.dict(
                    browser_reasoner.os.environ,
                    {browser_reasoner.OPT_IN_ENV: value}, clear=False):
                self.assertFalse(browser_reasoner.enabled())
    ...
    def test_local_browser_failure_never_echoes_prompt(self):
        ...
        with mock.patch.dict(
                browser_reasoner.os.environ,
                {browser_reasoner.OPT_IN_ENV: "1"}, clear=False), \
