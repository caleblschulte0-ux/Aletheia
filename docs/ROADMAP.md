...
| 6 | The one Thea page | **BUILT** v2 | `interface/thea.html` served by the Core at `/` (was `command.html`, consolidated 2026-09-18): approvals with approve/deny, live task queue, 15-kind command composer, HALT/RESUME, receipts — every button the same gated grammar as the intercom. The intercom remains the remote/voice channel |
...
| 23 | Agent Runtime v1 — she manages workers rather than routing to a model | **BUILT** v1 code — **UNPROVEN LIVE** | `aletheia/agents.py` (identity, scope, lifecycle, supervisor, and `act()` — the one door work passes through), `aletheia/teams.py` (bounded parallel workers, honest independence, disagreement reported rather than averaged), `aletheia/workspaces.py` (what a worker knows, kept apart from what it said), `aletheia/delegation.py` (whether to build a team at all — mostly refusals). 65 tests. The hierarchy holds by arithmetic: no agent may hold a capability the registry calls high-risk or operator_always (root reaches 104 of 130), a child is a subset of its parent, permission is re-checked at execution time, and `kill()` propagates downward. UNPROVEN because the three proof agents and the five acceptance tests need hours of live provider calls and have not been run — see `docs/AGENT_RUNTIME_V1.md`, which lists what is unfinished before what is done |
